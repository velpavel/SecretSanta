...
def ask_password():
    val = get_text_val(
        'Введите пароль для доступа к вашей группе. Только люди знающие этот пароль смогу участвовать с вами в тайном Санте')
    if val:
        group.password = val
        return 1
    return 0
...
def ask_date():
    msg = '''Введите дату окончания приёма заявок. В эту дату генератор случайных чисел распределит отправитель-получатель и вышлет участникам их адресатов
Дату необходимо воодить в формате дд.мм.гггг т.к. 15ое декабря 2016го года это 15.12.2016 '''
...
def finish():
    msg = '''Ваша группа "{}" создана. Пароль на группу: {}\n Дата распеределения: {} Приглашайте участников.
    Обратите внимание! Вы сами пока не участвуете в своей группе. Для участия Вам надо зарегеистрироваться в своей группе.'''.format(group.name, group.password, group.date_shuffle.strftime('%d.%m.%Y'))
    group.active = True
